...
class ConcatText:
    ...
    def run(self): #HANDLER        
        self.getFiles()
        self.filenameNew = input("new file name(result.txt)>>") or 'result.txt'                
        self.encodingOld = input("encodingOld(cp949)>>") or 'cp949'
        self.encodingNew = input("encodingNew(utf8)>>") or 'utf8'

        flag = input("헤더가 포함되어 있습니까?(기본값 Y). Y인 경우 첫번째 파일만 포함함)>>") or 'Y'
        if flag == 'Y': self.bHeader = True
        else: self.bHeader = False

        self.concat(self.bHeader)
        print("DONE")

    #231213: 하위폴더 순환하도록 변경
    def getFiles(self, msg:str = "합칠 텍스트 파일이 있는 경로명(하위폴더 포함)"): 
        ...
        for dirpath, dirname, filenames in os.walk(path):
            for file in filenames:
                ...
                if strContain != None:
                    if (path.split(".")[-1] == ext) and (strContain in path): self.liTgt.append(path)
                elif strContain == None:
                    if path.split(".")[-1] == ext: self.liTgt.append(path)

    def concat(self, bHeader:bool): #bHeader가 True면 헤더 포함
        # 합산파일 생성
        fileNew = open(self.filenameNew,'wt', encoding=self.encodingNew)
        fileNew.close()

        # 파일 하나씩 append        
        fileNew = open(self.filenameNew,'at', encoding=self.encodingNew)        
        print("총",len(self.liTgt),"개")
        pbar = tqdm.tqdm(total=len(self.liTgt) , desc="순환")
        totalj = 0
        i = 0 #i = File
        for file in self.liTgt:    
            bContainLF = self.checkLF(file, self.encodingOld) #파일을 열 때, 해당 파일 마지막에 LF를 포함하는지 확인한다.           

            j = 0 #j = Line           
            fileOld = open(file, 'rt', encoding=self.encodingOld)
            for line in fileOld:                
